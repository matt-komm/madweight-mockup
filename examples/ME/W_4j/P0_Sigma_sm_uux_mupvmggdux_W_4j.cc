...
#include "P0_Sigma_sm_uux_mupvmggdux_W_4j.h"
...
// Process: u u~ > w+ g g d u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c c~ > w+ g g s c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uux_mupvmggdux_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uux_mupvmggdux_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uux_wpggdux_wp_mupvm(); 
        ...
        t[1] = matrix_uux_wpggdux_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_uux_mupvmggdux_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uux_mupvmggdux_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[7], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[47]); 
  FFV1_1(w[1], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[48]); 
...
double P0_Sigma_sm_uux_mupvmggdux_W_4j::matrix_uux_wpggdux_wp_mupvm() 
{
  int i, j; 
  // Local variables
  ...
  jamp[7] = +1./2. * (+1./3. * std::complex<double> (0, 1) * amp[0] +
      ...
      amp[202]);
  jamp[8] = +1./2. * (-amp[48] - 1./3. * amp[49] - 1./3. * amp[50] - amp[51] -
      ...
      amp[201] - amp[202]);
  jamp[9] = +1./2. * (+1./3. * amp[48] + amp[49] + amp[50] + 1./3. * amp[51] +
