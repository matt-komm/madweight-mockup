...
#include "P0_Sigma_sm_uux_mumvmxggcsx_W_4j.h"
...
// Process: u u~ > w- g g c s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: c c~ > w- g g u d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d d~ > w- g g c s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s~ > w- g g u d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uux_mumvmxggcsx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uux_mumvmxggcsx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uux_wmggcsx_wm_mumvmx(); 
        ...
        t[1] = matrix_uux_wmggcsx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_uux_mumvmxggcsx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uux_mumvmxggcsx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[0], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
...
double P0_Sigma_sm_uux_mumvmxggcsx_W_4j::matrix_uux_wmggcsx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[6] = +1./2. * (+1./3. * std::complex<double> (0, 1) * amp[0] + 1./3. *
      ...
      amp[72] - 1./3. * amp[75]);
  jamp[7] = +1./2. * (-std::complex<double> (0, 1) * amp[0] + amp[3] -
  ...
  jamp[8] = +1./2. * (+amp[24] + amp[25] + amp[28] + amp[29] +
      ...
      amp[99] + amp[100]);
  jamp[9] = +1./2. * (-1./3. * amp[24] - 1./3. * amp[25] - 1./3. * amp[26] -
