...
#include "P0_Sigma_sm_usx_mupvmuccxcx_W_4j.h"
...
// Process: c d~ > w+ c u u~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: d s~ > w+ d c c~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s d~ > w+ s u u~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_usx_mupvmuccxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_usx_mupvmuccxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_usx_wpuccxcx_wp_mupvm(); 
        ...
        t[1] = matrix_usx_wpuccxcx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_usx_mupvmuccxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_usx_mupvmuccxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[7], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[18]); 
...
double P0_Sigma_sm_usx_mupvmuccxcx_W_4j::matrix_usx_wpuccxcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./4. * (-1./3. * amp[2] - 1./3. * amp[4] - amp[5] +
  ...
  jamp[1] = +1./4. * (+amp[1] - std::complex<double> (0, 1) * amp[3] + amp[4] +
