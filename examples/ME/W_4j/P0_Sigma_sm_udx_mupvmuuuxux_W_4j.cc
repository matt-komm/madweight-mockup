...
#include "P0_Sigma_sm_udx_mupvmuuuxux_W_4j.h"
...
void P0_Sigma_sm_udx_mupvmuuuxux_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_udx_mupvmuuuxux_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_udx_wpuuuxux_wp_mupvm(); 
        ...
        t[1] = matrix_udx_wpuuuxux_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_udx_mupvmuuuxux_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_udx_mupvmuuuxux_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_udx_mupvmuuuxux_W_4j::matrix_udx_wpuuuxux_wp_mupvm() 
{
  int i, j; 
  // Local variables
  ...
  jamp[1] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + 1./9. * amp[5] + 1./9.
      ...
      * amp[20] + 1./3. * amp[22] + 1./3. * amp[23] + 1./3. * amp[25] + 1./3. *
      ...
      amp[95]);
  jamp[2] = +1./4. * (+1./3. * amp[2] + 1./3. * amp[3] + amp[6] + amp[8] -
