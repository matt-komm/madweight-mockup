...
#include "P0_Sigma_sm_udx_mupvmcccxcx_W_4j.h"
...
// Process: u d~ > w+ c c c~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u d~ > w+ s s s~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s~ > w+ u u u~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s~ > w+ d d d~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_udx_mupvmcccxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_udx_mupvmcccxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_udx_wpcccxcx_wp_mupvm(); 
        ...
        t[1] = matrix_udx_wpcccxcx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_udx_mupvmcccxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_udx_mupvmcccxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[32]); 
  FFV1_2(w[0], w[17], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  FFV1_2(w[0], w[20], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  FFV1_1(w[1], w[20], pars->GC_11, pars->ZERO, pars->ZERO, w[35]); 
  FFV1_1(w[1], w[17], pars->GC_11, pars->ZERO, pars->ZERO, w[36]); 
...
double P0_Sigma_sm_udx_mupvmcccxcx_W_4j::matrix_udx_wpcccxcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
