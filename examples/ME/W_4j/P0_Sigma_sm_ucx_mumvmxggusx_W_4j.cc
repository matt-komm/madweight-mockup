...
#include "P0_Sigma_sm_ucx_mumvmxggusx_W_4j.h"
...
// Process: c u~ > w- g g c d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d c~ > w- g g d s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s u~ > w- g g s d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ucx_mumvmxggusx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ucx_mumvmxggusx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ucx_wmggusx_wm_mumvmx(); 
        ...
        t[1] = matrix_ucx_wmggusx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ucx_mumvmxggusx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ucx_mumvmxggusx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_ucx_mumvmxggusx_W_4j::matrix_ucx_wmggusx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[3] = +1./2. * (+1./3. * std::complex<double> (0, 1) * amp[0] + 1./3. *
      ...
      amp[60] + 1./3. * amp[63]);
  jamp[4] = +1./2. * (-amp[24] - amp[25] - amp[28] - amp[29] -
      ...
      amp[99] - amp[100]);
  jamp[5] = +1./2. * (+1./3. * amp[24] + 1./3. * amp[25] + 1./3. * amp[26] +
