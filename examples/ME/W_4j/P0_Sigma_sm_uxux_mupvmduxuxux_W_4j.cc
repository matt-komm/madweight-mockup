...
#include "P0_Sigma_sm_uxux_mupvmduxuxux_W_4j.h"
...
// Process: u~ u~ > w+ d u~ u~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c~ c~ > w+ s c~ c~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uxux_mupvmduxuxux_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uxux_mupvmduxuxux_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_wpduxuxux_wp_mupvm(); 
...
double P0_Sigma_sm_uxux_mupvmduxuxux_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uxux_mupvmduxuxux_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[5], w[27], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  ...
  FFV1_1(w[5], w[46], pars->GC_11, pars->ZERO, pars->ZERO, w[51]); 
  ...
  FFV1_0(w[22], w[5], w[17], pars->GC_11, amp[8]); 
  ...
  FFV1_0(w[28], w[5], w[33], pars->GC_11, amp[17]); 
  ...
  FFV1_0(w[28], w[5], w[50], pars->GC_11, amp[41]); 
...
double P0_Sigma_sm_uxux_mupvmduxuxux_W_4j::matrix_uxux_wpduxuxux_wp_mupvm() 
{
  int i, j; 
  // Local variables
