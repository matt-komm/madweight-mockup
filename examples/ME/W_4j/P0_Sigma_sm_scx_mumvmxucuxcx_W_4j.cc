...
#include "P0_Sigma_sm_scx_mumvmxucuxcx_W_4j.h"
...
// Process: s c~ > w- u c u~ c~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_scx_mumvmxucuxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_scx_mumvmxucuxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_scx_wmucuxcx_wm_mumvmx(); 
        ...
        t[1] = matrix_scx_wmucuxcx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_scx_mumvmxucuxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_scx_mumvmxucuxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[7], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[32]); 
  FFV1_1(w[5], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  FFV1_2(w[7], w[20], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  ...
  FFV1_0(w[7], w[33], w[29], pars->GC_11, amp[25]); 
  FFV1_0(w[32], w[5], w[31], pars->GC_11, amp[26]); 
  FFV1_0(w[7], w[33], w[31], pars->GC_11, amp[27]); 
  FFV1_0(w[34], w[5], w[28], pars->GC_11, amp[28]); 
  FFV1_0(w[7], w[35], w[28], pars->GC_11, amp[29]); 
  FFV1_0(w[34], w[5], w[30], pars->GC_11, amp[30]); 
  FFV1_0(w[7], w[35], w[30], pars->GC_11, amp[31]); 

}
double P0_Sigma_sm_scx_mumvmxucuxcx_W_4j::matrix_scx_wmucuxcx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[3] = +1./4. * (+amp[8] - std::complex<double> (0, 1) * amp[10] + amp[11]
