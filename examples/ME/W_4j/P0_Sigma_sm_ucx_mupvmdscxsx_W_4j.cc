...
#include "P0_Sigma_sm_ucx_mupvmdscxsx_W_4j.h"
...
// Process: u c~ > w+ d s c~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u s~ > w+ d c s~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c u~ > w+ s d u~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
...
void P0_Sigma_sm_ucx_mupvmdscxsx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ucx_mupvmdscxsx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ucx_wpdscxsx_wp_mupvm(); 
        ...
        t[1] = matrix_ucx_wpdscxsx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ucx_mupvmdscxsx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ucx_mupvmdscxsx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_1(w[5], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[19]); 
  ...
  FFV1_1(w[5], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[24]); 
  ...
  FFV1_0(w[17], w[1], w[20], pars->GC_11, amp[14]); 
  FFV1_0(w[7], w[18], w[20], pars->GC_11, amp[15]); 

}
double P0_Sigma_sm_ucx_mupvmdscxsx_W_4j::matrix_ucx_wpdscxsx_wp_mupvm() 
{
  int i, j; 
  // Local variables
