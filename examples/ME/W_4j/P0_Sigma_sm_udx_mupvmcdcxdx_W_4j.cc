...
#include "P0_Sigma_sm_udx_mupvmcdcxdx_W_4j.h"
...
// Process: u d~ > w+ c d c~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s~ > w+ u s u~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s~ > w+ d s d~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_udx_mupvmcdcxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_udx_mupvmcdcxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_udx_wpcdcxdx_wp_mupvm(); 
        ...
        t[1] = matrix_udx_wpcdcxdx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_udx_mupvmcdcxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_udx_mupvmcdcxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_1(w[1], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[25]); 
  ...
  FFV1_1(w[1], w[19], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  ...
  FFV1_0(w[13], w[1], w[30], pars->GC_11, amp[20]); 
...
double P0_Sigma_sm_udx_mupvmcdcxdx_W_4j::matrix_udx_wpcdcxdx_wp_mupvm() 
{
  int i, j; 
  // Local variables
