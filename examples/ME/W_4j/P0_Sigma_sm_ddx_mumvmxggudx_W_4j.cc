...
#include "P0_Sigma_sm_ddx_mumvmxggudx_W_4j.h"
...
void P0_Sigma_sm_ddx_mumvmxggudx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ddx_mumvmxggudx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_wmggudx_wm_mumvmx(); 
        ...
        t[1] = matrix_ddx_wmggudx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ddx_mumvmxggudx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ddx_mumvmxggudx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[0], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  ...
  FFV1_3(w[61], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[62]); 
  ...
  FFV1_3(w[68], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[69]); 
  ...
  FFV1_0(w[39], w[1], w[73], pars->GC_11, amp[106]); 
...
double P0_Sigma_sm_ddx_mumvmxggudx_W_4j::matrix_ddx_wmggudx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./2. * (+std::complex<double> (0, 1) * amp[3] + 1./3. *
  ...
  jamp[2] = +1./2. * (-std::complex<double> (0, 1) * amp[2] -
      ...
      * amp[47] + amp[144] + amp[145] - std::complex<double> (0, 1) * amp[146]
      ...
      amp[201]);
  jamp[3] = +1./2. * (+std::complex<double> (0, 1) * amp[2] +
      std::complex<double> (0, 1) * amp[4] + amp[5] + 1./3. *
      std::complex<double> (0, 1) * amp[7] + 1./3. * std::complex<double> (0,
      1) * amp[9] + 1./3. * std::complex<double> (0, 1) * amp[12] + 1./3. *
      std::complex<double> (0, 1) * amp[13] + std::complex<double> (0, 1) *
      ...
      amp[202]);
  jamp[4] = +1./2. * (-1./3. * amp[48] - amp[49] - amp[50] - 1./3. * amp[51] -
      ...
      amp[193]);
  jamp[5] = +1./2. * (+amp[48] + 1./3. * amp[49] + 1./3. * amp[50] + amp[51] +
      ...
      amp[201] + amp[202]);
  jamp[6] = +1./2. * (+std::complex<double> (0, 1) * amp[0] + 1./3. *
  ...
  jamp[7] = +1./2. * (-1./3. * std::complex<double> (0, 1) * amp[0] -
      std::complex<double> (0, 1) * amp[1] - 1./3. * std::complex<double> (0,
      ...
      amp[202]);
  jamp[8] = +1./2. * (+amp[24] + 1./3. * amp[25] + 1./3. * amp[26] + amp[27] +
      ...
      amp[201] + amp[202]);
  jamp[9] = +1./2. * (-1./3. * amp[24] - amp[25] - amp[26] - 1./3. * amp[27] -
