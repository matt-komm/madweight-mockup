...
#include "P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j.h"
...
// Process: u~ c~ > w- c c~ c~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: u~ s~ > w- s s~ d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: c~ d~ > w- d d~ d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxcx_wmccxcxdx_wm_mumvmx(); 
        ...
        t[1] = matrix_uxcx_wmccxcxdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[0], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
  FFV2_1(w[0], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[15]); 
  ...
  FFV1_0(w[18], w[0], w[16], pars->GC_11, amp[5]); 
  FFV1_0(w[19], w[0], w[10], pars->GC_11, amp[6]); 
  ...
  FFV1_0(w[18], w[0], w[30], pars->GC_11, amp[17]); 
  FFV1_0(w[32], w[0], w[25], pars->GC_11, amp[18]); 
...
double P0_Sigma_sm_uxcx_mumvmxccxcxdx_W_4j::matrix_uxcx_wmccxcxdx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
