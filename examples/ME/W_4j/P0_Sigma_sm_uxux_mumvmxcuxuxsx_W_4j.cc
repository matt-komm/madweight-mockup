...
#include "P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j.h"
...
// Process: u~ u~ > w- c u~ u~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: c~ c~ > w- u c~ c~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d~ d~ > w- c d~ d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s~ s~ > w- u s~ d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_wmcuxuxsx_wm_mumvmx(); 
...
double P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[25], pars->GC_11, pars->ZERO, pars->ZERO, w[36]); 
  ...
  FFV1_0(w[35], w[1], w[21], pars->GC_11, amp[24]); 
  FFV1_0(w[6], w[36], w[21], pars->GC_11, amp[25]); 
  FFV1_0(w[35], w[1], w[23], pars->GC_11, amp[26]); 
...
double P0_Sigma_sm_uxux_mumvmxcuxuxsx_W_4j::matrix_uxux_wmcuxuxsx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
