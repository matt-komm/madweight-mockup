...
#include "P0_Sigma_sm_usx_mupvmuuuxcx_W_4j.h"
...
// Process: c d~ > w+ c c c~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: d s~ > w+ d d d~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s d~ > w+ s s s~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_usx_mupvmuuuxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_usx_mupvmuuuxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_usx_wpuuuxcx_wp_mupvm(); 
        ...
        t[1] = matrix_usx_wpuuuxcx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_usx_mupvmuuuxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_usx_mupvmuuuxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_usx_mupvmuuuxcx_W_4j::matrix_usx_wpuuuxcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
