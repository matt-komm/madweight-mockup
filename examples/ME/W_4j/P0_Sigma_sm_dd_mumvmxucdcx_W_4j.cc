...
#include "P0_Sigma_sm_dd_mumvmxucdcx_W_4j.h"
...
// Process: d d > w- u c d c~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d d > w- u s d s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s > w- c u s u~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s > w- c d s d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dd_mumvmxucdcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dd_mumvmxucdcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dd_wmucdcx_wm_mumvmx(); 
...
double P0_Sigma_sm_dd_mumvmxucdcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dd_mumvmxucdcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[8], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[9]); 
  ...
  FFV1_2(w[8], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[32]); 
  FFV1_1(w[6], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  FFV1_2(w[8], w[18], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  ...
  FFV1_0(w[8], w[33], w[12], pars->GC_11, amp[25]); 
  FFV1_0(w[34], w[6], w[27], pars->GC_11, amp[26]); 
  FFV1_0(w[8], w[35], w[27], pars->GC_11, amp[27]); 
  FFV1_0(w[34], w[6], w[29], pars->GC_11, amp[28]); 
  FFV1_0(w[8], w[35], w[29], pars->GC_11, amp[29]); 
  FFV1_0(w[32], w[6], w[28], pars->GC_11, amp[30]); 
  FFV1_0(w[8], w[33], w[28], pars->GC_11, amp[31]); 

}
double P0_Sigma_sm_dd_mumvmxucdcx_W_4j::matrix_dd_wmucdcx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./4. * (-1./3. * amp[0] - 1./3. * amp[1] - amp[6] -
