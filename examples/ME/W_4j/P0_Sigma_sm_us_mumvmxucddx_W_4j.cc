...
#include "P0_Sigma_sm_us_mumvmxucddx_W_4j.h"
...
// Process: u s > w- u c d d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: c d > w- c u s s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d s > w- d c u u~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_us_mumvmxucddx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_us_mumvmxucddx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_us_wmucddx_wm_mumvmx(); 
        ...
        t[1] = matrix_us_wmucddx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_us_mumvmxucddx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_us_mumvmxucddx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[5], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[25]); 
  ...
  FFV1_0(w[24], w[5], w[21], pars->GC_11, amp[12]); 
  FFV1_0(w[0], w[25], w[21], pars->GC_11, amp[13]); 
  FFV1_0(w[24], w[5], w[23], pars->GC_11, amp[14]); 
  FFV1_0(w[0], w[25], w[23], pars->GC_11, amp[15]); 

}
double P0_Sigma_sm_us_mumvmxucddx_W_4j::matrix_us_wmucddx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
