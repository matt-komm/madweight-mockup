...
#include "P0_Sigma_sm_uu_mupvmuuscx_W_4j.h"
...
// Process: u u > w+ u u s c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c c > w+ c c d u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: d d > w+ d d s c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s s > w+ s d s u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uu_mupvmuuscx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uu_mupvmuuscx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uu_wpuuscx_wp_mupvm(); 
...
double P0_Sigma_sm_uu_mupvmuuscx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uu_mupvmuuscx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_uu_mupvmuuscx_W_4j::matrix_uu_wpuuscx_wp_mupvm() 
{
  int i, j; 
  // Local variables
