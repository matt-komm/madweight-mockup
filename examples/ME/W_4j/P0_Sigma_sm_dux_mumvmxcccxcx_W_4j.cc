...
#include "P0_Sigma_sm_dux_mumvmxcccxcx_W_4j.h"
...
// Process: d u~ > w- c c c~ c~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d u~ > w- s s s~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s c~ > w- u u u~ u~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s c~ > w- d d d~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dux_mumvmxcccxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dux_mumvmxcccxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dux_wmcccxcx_wm_mumvmx(); 
        ...
        t[1] = matrix_dux_wmcccxcx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_dux_mumvmxcccxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dux_mumvmxcccxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_dux_mumvmxcccxcx_W_4j::matrix_dux_wmcccxcx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[1] = +1./4. * (-1./3. * amp[8] - 1./3. * amp[9] - 1./3. * amp[10] -
