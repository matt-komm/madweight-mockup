...
#include "P0_Sigma_sm_dux_mumvmxggddx_W_4j.h"
...
// Process: d u~ > w- g g d d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s c~ > w- g g s s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dux_mumvmxggddx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dux_mumvmxggddx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dux_wmggddx_wm_mumvmx(); 
        ...
        t[1] = matrix_dux_wmggddx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_dux_mumvmxggddx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dux_mumvmxggddx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[8], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[19]); 
  ...
  FFV1_1(w[1], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[49]); 
  ...
  FFV1_3(w[61], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[62]); 
  ...
  FFV1_3(w[68], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[69]); 
  ...
  FFV1_0(w[39], w[7], w[73], pars->GC_11, amp[106]); 
...
double P0_Sigma_sm_dux_mumvmxggddx_W_4j::matrix_dux_wmggddx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[6] = +1./2. * (+std::complex<double> (0, 1) * amp[2] +
      ...
      * amp[47] - amp[144] - amp[145] + std::complex<double> (0, 1) * amp[146]
      ...
      amp[201]);
  jamp[7] = +1./2. * (-std::complex<double> (0, 1) * amp[3] - 1./3. *
  ...
  jamp[8] = +1./2. * (+1./3. * amp[48] + amp[49] + amp[50] + 1./3. * amp[51] +
      ...
      amp[193]);
  jamp[9] = +1./2. * (-amp[48] - 1./3. * amp[49] - 1./3. * amp[50] - amp[51] -
