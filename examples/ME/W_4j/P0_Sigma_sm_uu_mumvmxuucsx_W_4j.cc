...
#include "P0_Sigma_sm_uu_mumvmxuucsx_W_4j.h"
...
// Process: u u > w- u u c s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: c c > w- c u c d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d d > w- d c d s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s > w- s u s d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uu_mumvmxuucsx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uu_mumvmxuucsx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uu_wmuucsx_wm_mumvmx(); 
...
double P0_Sigma_sm_uu_mumvmxuucsx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uu_mumvmxuucsx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[1], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[25]); 
  ...
  FFV1_2(w[1], w[24], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  ...
  FFV1_0(w[1], w[34], w[21], pars->GC_11, amp[21]); 
  FFV1_0(w[33], w[5], w[23], pars->GC_11, amp[22]); 
  FFV1_0(w[1], w[34], w[23], pars->GC_11, amp[23]); 
...
double P0_Sigma_sm_uu_mumvmxuucsx_W_4j::matrix_uu_wmuucsx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
