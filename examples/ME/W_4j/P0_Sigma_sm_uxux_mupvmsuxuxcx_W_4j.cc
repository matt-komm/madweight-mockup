...
#include "P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j.h"
...
// Process: u~ u~ > w+ s u~ u~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c~ c~ > w+ d c~ u~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: d~ d~ > w+ s d~ c~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s~ s~ > w+ d s~ u~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uxux_wpsuxuxcx_wp_mupvm(); 
...
double P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_uxux_mupvmsuxuxcx_W_4j::matrix_uxux_wpsuxuxcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
