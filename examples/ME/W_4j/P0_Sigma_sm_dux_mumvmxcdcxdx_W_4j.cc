...
#include "P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j.h"
...
// Process: d u~ > w- c d c~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s c~ > w- u s u~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s c~ > w- d s d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dux_wmcdcxdx_wm_mumvmx(); 
        ...
        t[1] = matrix_dux_wmcdcxdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_dux_mumvmxcdcxdx_W_4j::matrix_dux_wmcdcxdx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
