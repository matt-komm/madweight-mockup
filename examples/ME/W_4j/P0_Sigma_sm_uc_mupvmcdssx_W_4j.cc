...
#include "P0_Sigma_sm_uc_mupvmcdssx_W_4j.h"
...
// Process: u c > w+ c d s s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u s > w+ s d c c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c d > w+ s u d u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uc_mupvmcdssx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uc_mupvmcdssx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_uc_wpcdssx_wp_mupvm(); 
        ...
        t[1] = matrix_uc_wpcdssx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_uc_mupvmcdssx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uc_mupvmcdssx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_1(w[6], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[19]); 
  ...
  FFV1_1(w[6], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[24]); 
  ...
  FFV1_0(w[13], w[7], w[20], pars->GC_11, amp[8]); 
  ...
  FFV1_0(w[1], w[18], w[20], pars->GC_11, amp[15]); 

}
double P0_Sigma_sm_uc_mupvmcdssx_W_4j::matrix_uc_wpcdssx_wp_mupvm() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./4. * (+1./3. * amp[2] + 1./3. * amp[4] + 1./3. * amp[5] + 1./3.
