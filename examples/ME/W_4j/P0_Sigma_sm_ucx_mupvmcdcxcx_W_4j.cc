...
#include "P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j.h"
...
// Process: u c~ > w+ c d c~ c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u s~ > w+ s d s~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c u~ > w+ u s u~ u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c d~ > w+ d s d~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ucx_wpcdcxcx_wp_mupvm(); 
        ...
        t[1] = matrix_ucx_wpcdcxcx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[6], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[32]); 
  FFV1_2(w[0], w[17], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  FFV1_2(w[0], w[20], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  FFV1_1(w[6], w[20], pars->GC_11, pars->ZERO, pars->ZERO, w[35]); 
  FFV1_1(w[6], w[17], pars->GC_11, pars->ZERO, pars->ZERO, w[36]); 
  ...
  FFV1_0(w[13], w[5], w[28], pars->GC_11, amp[15]); 
  ...
  FFV1_0(w[19], w[1], w[28], pars->GC_11, amp[22]); 
  ...
  FFV1_0(w[23], w[5], w[28], pars->GC_11, amp[28]); 
  FFV1_0(w[7], w[24], w[28], pars->GC_11, amp[29]); 
  FFV1_0(w[25], w[1], w[28], pars->GC_11, amp[30]); 
  FFV1_0(w[7], w[26], w[28], pars->GC_11, amp[31]); 

}
double P0_Sigma_sm_ucx_mupvmcdcxcx_W_4j::matrix_ucx_wpcdcxcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
