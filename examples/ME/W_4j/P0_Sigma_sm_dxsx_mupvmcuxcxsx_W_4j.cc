...
#include "P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j.h"
...
// Process: d~ s~ > w+ c u~ c~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dxsx_wpcuxcxsx_wp_mupvm(); 
        ...
        t[1] = matrix_dxsx_wpcuxcxsx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_1(w[0], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[19]); 
  FFV1_3(w[6], w[19], pars->GC_11, pars->ZERO, pars->ZERO, w[20]); 
  FFV1_2(w[6], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[21]); 
  FFV1_2(w[6], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[22]); 
  FFV1_1(w[0], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[23]); 
  FFV1_1(w[0], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[24]); 
  ...
  FFV1_0(w[13], w[1], w[20], pars->GC_11, amp[8]); 
...
double P0_Sigma_sm_dxsx_mupvmcuxcxsx_W_4j::matrix_dxsx_wpcuxcxsx_wp_mupvm() 
{
  int i, j; 
  // Local variables
