...
#include "P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j.h"
...
// Process: d~ d~ > w+ c u~ c~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: d~ d~ > w+ s u~ s~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s~ s~ > w+ u c~ u~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: s~ s~ > w+ d c~ d~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_dxdx_wpcuxcxdx_wp_mupvm(); 
...
double P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_1(w[0], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[25]); 
  FFV1_3(w[6], w[25], pars->GC_11, pars->ZERO, pars->ZERO, w[26]); 
  FFV1_2(w[6], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[27]); 
  FFV1_2(w[6], w[19], pars->GC_11, pars->ZERO, pars->ZERO, w[28]); 
  FFV2_1(w[1], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[29]); 
  FFV1_3(w[6], w[29], pars->GC_11, pars->ZERO, pars->ZERO, w[30]); 
  FFV1_2(w[6], w[16], pars->GC_11, pars->ZERO, pars->ZERO, w[31]); 
  FFV1_1(w[1], w[16], pars->GC_11, pars->ZERO, pars->ZERO, w[32]); 
  FFV1_1(w[0], w[19], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  ...
  FFV1_0(w[13], w[0], w[30], pars->GC_11, amp[20]); 
  ...
  FFV1_0(w[7], w[24], w[26], pars->GC_11, amp[29]); 
  FFV1_0(w[21], w[5], w[30], pars->GC_11, amp[30]); 
  FFV1_0(w[7], w[22], w[30], pars->GC_11, amp[31]); 

}
double P0_Sigma_sm_dxdx_mupvmcuxcxdx_W_4j::matrix_dxdx_wpcuxcxdx_wp_mupvm() 
{
  int i, j; 
  // Local variables
