...
#include "P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j.h"
...
void P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gdx_wmgudxdx_wm_mumvmx(); 
        ...
        t[1] = matrix_gdx_wmgudxdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[48]); 
...
double P0_Sigma_sm_gdx_mumvmxgudxdx_W_4j::matrix_gdx_wmgudxdx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./2. * (-1./3. * amp[48] - amp[49] - amp[50] - 1./3. * amp[51] -
      ...
      amp[193]);
  jamp[1] = +1./2. * (+amp[48] + 1./3. * amp[49] + 1./3. * amp[50] + amp[51] +
  ...
  jamp[6] = +1./2. * (+amp[24] + 1./3. * amp[25] + 1./3. * amp[26] + amp[27] +
      ...
      amp[201] + amp[202]);
  jamp[7] = +1./2. * (-1./3. * amp[24] - amp[25] - amp[26] - 1./3. * amp[27] -
