...
#include "P0_Sigma_sm_udx_mupvmuduxdx_W_4j.h"
...
// Process: u d~ > w+ u d u~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s~ > w+ c s c~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_udx_mupvmuduxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_udx_mupvmuduxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_udx_wpuduxdx_wp_mupvm(); 
        ...
        t[1] = matrix_udx_wpuduxdx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_udx_mupvmuduxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_udx_mupvmuduxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_udx_mupvmuduxdx_W_4j::matrix_udx_wpuduxdx_wp_mupvm() 
{
  int i, j; 
  // Local variables
  ...
  jamp[1] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + amp[3] + amp[5] -
  ...
  jamp[4] = +1./4. * (-amp[0] - 1./3. * amp[4] - 1./3. * amp[5] - amp[7] -
