...
#include "P0_Sigma_sm_ddx_mumvmxuccxdx_W_4j.h"
...
// Process: d d~ > w- u c c~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: d d~ > w- u s s~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s~ > w- c u u~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: s s~ > w- c d d~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ddx_mumvmxuccxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ddx_mumvmxuccxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_wmuccxdx_wm_mumvmx(); 
        ...
        t[1] = matrix_ddx_wmuccxdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ddx_mumvmxuccxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ddx_mumvmxuccxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[7], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[9]); 
  ...
  FFV1_1(w[6], w[14], pars->GC_11, pars->ZERO, pars->ZERO, w[33]); 
  FFV1_2(w[7], w[18], pars->GC_11, pars->ZERO, pars->ZERO, w[34]); 
  FFV1_1(w[6], w[18], pars->GC_11, pars->ZERO, pars->ZERO, w[35]); 
