...
#include "P0_Sigma_sm_ud_mupvmcddcx_W_4j.h"
...
// Process: u d > w+ c d d c~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u d > w+ s d d s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s > w+ u s s u~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c s > w+ d s s d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ud_mupvmcddcx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ud_mupvmcddcx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ud_wpcddcx_wp_mupvm(); 
        ...
        t[1] = matrix_ud_wpcddcx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ud_mupvmcddcx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ud_mupvmcddcx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_0(w[1], w[15], w[30], pars->GC_11, amp[19]); 
...
double P0_Sigma_sm_ud_mupvmcddcx_W_4j::matrix_ud_wpcddcx_wp_mupvm() 
{
  int i, j; 
  // Local variables
