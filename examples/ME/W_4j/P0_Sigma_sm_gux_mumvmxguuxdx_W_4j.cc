...
#include "P0_Sigma_sm_gux_mumvmxguuxdx_W_4j.h"
...
// Process: g u~ > w- g u u~ d~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: g c~ > w- g c c~ s~ WEIGHTED=6
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_gux_mumvmxguuxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gux_mumvmxguuxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gux_wmguuxdx_wm_mumvmx(); 
        ...
        t[1] = matrix_gux_wmguuxdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_gux_mumvmxguuxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_gux_mumvmxguuxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_gux_mumvmxguuxdx_W_4j::matrix_gux_wmguuxdx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./2. * (-amp[24] - 1./3. * amp[25] - amp[26] - 1./3. * amp[27] -
      ...
      * amp[92] - amp[94] + std::complex<double> (0, 1) * amp[95] - amp[133] +
      ...
      amp[202]);
  jamp[1] = +1./2. * (+1./3. * amp[24] + amp[25] + 1./3. * amp[26] + amp[27] +
  ...
  jamp[5] = +1./2. * (+std::complex<double> (0, 1) * amp[0] - amp[3] +
