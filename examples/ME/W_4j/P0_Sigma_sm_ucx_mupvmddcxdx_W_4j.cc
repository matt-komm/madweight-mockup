...
#include "P0_Sigma_sm_ucx_mupvmddcxdx_W_4j.h"
...
// Process: u c~ > w+ d d c~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: u s~ > w+ d d s~ d~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c u~ > w+ s s u~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2
// Process: c d~ > w+ s s d~ s~ WEIGHTED=6
// *   Decay: w+ > mu+ vm WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ucx_mupvmddcxdx_W_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ucx_mupvmddcxdx_W_4j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ucx_wpddcxdx_wp_mupvm(); 
        ...
        t[1] = matrix_ucx_wpddcxdx_wp_mupvm(); 
        double tsum = 0; 
...
double P0_Sigma_sm_ucx_mupvmddcxdx_W_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ucx_mupvmddcxdx_W_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_ucx_mupvmddcxdx_W_4j::matrix_ucx_wpddcxdx_wp_mupvm() 
{
  int i, j; 
  // Local variables
