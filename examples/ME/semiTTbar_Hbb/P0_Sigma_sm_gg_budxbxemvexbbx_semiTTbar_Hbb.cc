...
#include "P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb.h"
...
void P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gg_ttxh_t_budx_tx_bxemvex_h_bbx(); 
...
double P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb::sigmaHat() 
{
...
void P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFS4_1(w[6], w[14], pars->GC_94, pars->MT, pars->WT, w[16]); 
  FFS4_2(w[11], w[14], pars->GC_94, pars->MT, pars->WT, w[17]); 
  FFV1_1(w[6], w[0], pars->GC_11, pars->MT, pars->WT, w[18]); 
  FFV1_2(w[11], w[1], pars->GC_11, pars->MT, pars->WT, w[19]); 
  FFV1_2(w[11], w[0], pars->GC_11, pars->MT, pars->WT, w[20]); 
  FFV1_1(w[6], w[1], pars->GC_11, pars->MT, pars->WT, w[21]); 
...
double P0_Sigma_sm_gg_budxbxemvexbbx_semiTTbar_Hbb::matrix_gg_ttxh_t_budx_tx_bxemvex_h_bbx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 8; 
