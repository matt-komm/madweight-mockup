...
#include "P0_Sigma_sm_gux_gux_qcd_2j.h"
...
void P0_Sigma_sm_gux_gux_qcd_2j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gux_gux_qcd_2j::sigmaKin() 
{
...
double P0_Sigma_sm_gux_gux_qcd_2j::sigmaHat() 
{
...
void P0_Sigma_sm_gux_gux_qcd_2j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  VVV1_1(w[0], w[2], pars->GC_10, pars->ZERO, pars->ZERO, w[4]); 
  FFV1_1(w[1], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[5]); 
  FFV1_2(w[3], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[6]); 

  // Calculate all amplitudes
  // Amplitude(s) for diagram number 0
  FFV1_0(w[3], w[1], w[4], pars->GC_11, amp[0]); 
  FFV1_0(w[3], w[5], w[2], pars->GC_11, amp[1]); 
  FFV1_0(w[6], w[1], w[2], pars->GC_11, amp[2]); 

}
double P0_Sigma_sm_gux_gux_qcd_2j::matrix_gux_gux() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 3; 
