...
#include "P0_Sigma_sm_ucx_uccxcx_qcd_4j.h"
...
void P0_Sigma_sm_ucx_uccxcx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ucx_uccxcx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_ucx_uccxcx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ucx_uccxcx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double P0_Sigma_sm_ucx_uccxcx_qcd_4j::matrix_ucx_uccxcx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./4. * (-amp[3] + std::complex<double> (0, 1) * amp[5] - amp[6] -
      1./3. * amp[8] - 1./3. * amp[9] - 1./3. * amp[10] - 1./3. * amp[11] -
      amp[13]);
  jamp[1] = +1./4. * (+amp[1] + std::complex<double> (0, 1) * amp[2] + 1./3. *
      amp[3] + 1./3. * amp[4] + amp[9] + amp[10] + 1./3. * amp[12] + 1./3. *
      amp[13]);
  jamp[2] = +1./4. * (+1./9. * amp[0] + 1./9. * amp[1] + 1./3. * amp[3] + 1./3.
      * amp[4] + 1./3. * amp[6] + 1./3. * amp[7] + 1./9. * amp[8] + 1./9. *
      amp[9] + 1./9. * amp[10] + 1./9. * amp[11]);
  jamp[3] = +1./4. * (-1./3. * amp[0] - 1./3. * amp[1] - amp[4] -
      std::complex<double> (0, 1) * amp[5] - amp[7] - 1./3. * amp[10] - 1./3. *
      amp[11] - amp[12]);
  jamp[4] = +1./4. * (-1./3. * amp[0] - 1./3. * amp[1] - 1./9. * amp[3] - 1./9.
      * amp[4] - 1./9. * amp[6] - 1./9. * amp[7] - 1./3. * amp[8] - 1./3. *
      amp[9] - 1./9. * amp[12] - 1./9. * amp[13]);
  jamp[5] = +1./4. * (+amp[0] - std::complex<double> (0, 1) * amp[2] + 1./3. *
