...
#include "P0_Sigma_sm_uux_gguux_qcd_4j.h"
...
void P0_Sigma_sm_uux_gguux_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uux_gguux_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_uux_gguux_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uux_gguux_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[0], w[4], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  FFV1_1(w[1], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
  FFV1_1(w[1], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_1(w[4], w[3], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  ...
  FFV1_1(w[4], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[22]); 
  FFV1_1(w[1], w[3], pars->GC_11, pars->ZERO, pars->ZERO, w[23]); 
...
double P0_Sigma_sm_uux_gguux_qcd_4j::matrix_uux_gguux() 
{
  int i, j; 
  // Local variables
  ...
  jamp[7] = +1./2. * (+std::complex<double> (0, 1) * amp[1] - amp[2] + 1./3. *
      std::complex<double> (0, 1) * amp[6] + 1./3. * std::complex<double> (0,
  ...
  jamp[8] = +1./2. * (-1./3. * amp[20] - amp[21] - 1./3. * amp[24] - amp[25] -
  ...
  jamp[9] = +1./2. * (+amp[20] + 1./3. * amp[21] + 1./3. * amp[22] + amp[23] +
