...
#include "P0_Sigma_sm_gux_gcuxcx_qcd_4j.h"
...
void P0_Sigma_sm_gux_gcuxcx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gux_gcuxcx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_gux_gcuxcx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_gux_gcuxcx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  FFV1_1(w[3], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
  FFV1_3(w[4], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_2(w[4], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  ...
  FFV1_1(w[1], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[20]); 
...
double P0_Sigma_sm_gux_gcuxcx_qcd_4j::matrix_gux_gcuxcx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[0] = +1./2. * (+1./3. * amp[5] + 1./3. * amp[7] + 1./3. * amp[16] +
      1./3. * amp[17]);
  jamp[1] = +1./2. * (-amp[5] - amp[6] - std::complex<double> (0, 1) * amp[9] -
  ...
  jamp[8] = +1./2. * (+1./3. * std::complex<double> (0, 1) * amp[0] + 1./3. *
      ...
      + 1./3. * amp[27]);
  jamp[9] = +1./2. * (-std::complex<double> (0, 1) * amp[0] - amp[2] -
