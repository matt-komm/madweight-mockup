...
#include "P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j.h"
...
// Process: u~ c~ > c u~ c~ c~ WEIGHTED=4
// Process: u~ d~ > d u~ d~ d~ WEIGHTED=4
// Process: u~ s~ > s u~ s~ s~ WEIGHTED=4
// Process: c~ d~ > d c~ d~ d~ WEIGHTED=4
// Process: c~ s~ > s c~ s~ s~ WEIGHTED=4
// Process: d~ s~ > s d~ s~ s~ WEIGHTED=4

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[3], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_2(w[3], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  FFV1_2(w[3], w[11], pars->GC_11, pars->ZERO, pars->ZERO, w[17]); 
  FFV1_2(w[3], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[18]); 
  ...
  FFV1_0(w[15], w[0], w[10], pars->GC_11, amp[10]); 
  FFV1_0(w[16], w[0], w[7], pars->GC_11, amp[11]); 
  FFV1_0(w[17], w[0], w[13], pars->GC_11, amp[12]); 
  FFV1_0(w[18], w[0], w[11], pars->GC_11, amp[13]); 

}
double P0_Sigma_sm_uxcx_cuxcxcx_qcd_4j::matrix_uxcx_cuxcxcx() 
{
  int i, j; 
  // Local variables
