...
#include "P0_Sigma_sm_ccx_ucuxcx_qcd_4j.h"
...
void P0_Sigma_sm_ccx_ucuxcx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ccx_ucuxcx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_ccx_ucuxcx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ccx_ucuxcx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[4], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[6]); 
  ...
  FFV1_3(w[0], w[3], pars->GC_11, pars->ZERO, pars->ZERO, w[11]); 
  FFV1_1(w[1], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[12]); 
  FFV1_3(w[5], w[1], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  FFV1_2(w[0], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
  FFV1_2(w[4], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_2(w[4], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  FFV1_2(w[4], w[11], pars->GC_11, pars->ZERO, pars->ZERO, w[17]); 
  FFV1_2(w[4], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[18]); 
  ...
  FFV1_0(w[0], w[8], w[13], pars->GC_11, amp[7]); 
  FFV1_0(w[14], w[1], w[10], pars->GC_11, amp[8]); 
  FFV1_0(w[0], w[12], w[10], pars->GC_11, amp[9]); 
  FFV1_0(w[15], w[2], w[10], pars->GC_11, amp[10]); 
  FFV1_0(w[16], w[2], w[7], pars->GC_11, amp[11]); 
  FFV1_0(w[17], w[2], w[13], pars->GC_11, amp[12]); 
  FFV1_0(w[18], w[2], w[11], pars->GC_11, amp[13]); 

}
double P0_Sigma_sm_ccx_ucuxcx_qcd_4j::matrix_ccx_ucuxcx() 
{
  int i, j; 
  // Local variables
  ...
  jamp[1] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + amp[4] +
  ...
  jamp[2] = +1./4. * (+amp[3] - std::complex<double> (0, 1) * amp[5] + amp[6] +
      ...
      amp[13]);
  jamp[3] = +1./4. * (-amp[1] - std::complex<double> (0, 1) * amp[2] - 1./3. *
      amp[3] - 1./3. * amp[4] - amp[9] - amp[10] - 1./3. * amp[12] - 1./3. *
      amp[13]);
  jamp[4] = +1./4. * (-amp[0] + std::complex<double> (0, 1) * amp[2] - 1./3. *
      amp[6] - 1./3. * amp[7] - amp[8] - amp[11] - 1./3. * amp[12] - 1./3. *
      amp[13]);
  jamp[5] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + 1./9. * amp[3] + 1./9.
