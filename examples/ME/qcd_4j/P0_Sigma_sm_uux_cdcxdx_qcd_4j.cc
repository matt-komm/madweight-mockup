...
#include "P0_Sigma_sm_uux_cdcxdx_qcd_4j.h"
...
// Process: u u~ > c d c~ d~ WEIGHTED=4
// Process: u u~ > c s c~ s~ WEIGHTED=4
// Process: u u~ > d s d~ s~ WEIGHTED=4
// Process: c c~ > u d u~ d~ WEIGHTED=4
// Process: c c~ > u s u~ s~ WEIGHTED=4
// Process: c c~ > d s d~ s~ WEIGHTED=4
// Process: d d~ > u s u~ s~ WEIGHTED=4
// Process: d d~ > c s c~ s~ WEIGHTED=4

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uux_cdcxdx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uux_cdcxdx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_uux_cdcxdx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uux_cdcxdx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_0(w[13], w[1], w[10], pars->GC_11, amp[5]); 
  FFV1_0(w[14], w[1], w[7], pars->GC_11, amp[6]); 

}
double P0_Sigma_sm_uux_cdcxdx_qcd_4j::matrix_uux_cdcxdx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 7; 
  ...
  jamp[1] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + 1./3. * amp[3] + 1./3.
      * amp[4]);
  jamp[2] = +1./4. * (+1./3. * amp[3] + 1./3. * amp[4] + 1./3. * amp[5] + 1./3.
      * amp[6]);
