...
#include "P0_Sigma_sm_ddx_ucuxcx_qcd_4j.h"
...
// Process: d d~ > u c u~ c~ WEIGHTED=4
// Process: s s~ > u c u~ c~ WEIGHTED=4
// Process: s s~ > u d u~ d~ WEIGHTED=4
// Process: s s~ > c d c~ d~ WEIGHTED=4

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_ddx_ucuxcx_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_ddx_ucuxcx_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_ddx_ucuxcx_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_ddx_ucuxcx_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_2(w[4], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  FFV1_2(w[4], w[10], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
...
double P0_Sigma_sm_ddx_ucuxcx_qcd_4j::matrix_ddx_ucuxcx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 7; 
  ...
  jamp[1] = +1./4. * (+1./3. * amp[3] + 1./3. * amp[4] + 1./3. * amp[5] + 1./3.
      * amp[6]);
