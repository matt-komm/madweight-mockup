...
#include "P0_Sigma_sm_uc_uucux_qcd_4j.h"
...
// Process: u c > u u c u~ WEIGHTED=4
// Process: u d > u u d u~ WEIGHTED=4
// Process: u s > u u s u~ WEIGHTED=4
// Process: c d > c c d c~ WEIGHTED=4
// Process: c s > c c s c~ WEIGHTED=4
// Process: d s > d d s d~ WEIGHTED=4

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_uc_uucux_qcd_4j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_uc_uucux_qcd_4j::sigmaKin() 
{
...
double P0_Sigma_sm_uc_uucux_qcd_4j::sigmaHat() 
{
...
void P0_Sigma_sm_uc_uucux_qcd_4j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_3(w[5], w[3], pars->GC_11, pars->ZERO, pars->ZERO, w[7]); 
  FFV1_1(w[4], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[8]); 
  FFV1_2(w[1], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[9]); 
  FFV1_3(w[1], w[4], pars->GC_11, pars->ZERO, pars->ZERO, w[10]); 
  FFV1_2(w[5], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[11]); 
  FFV1_1(w[3], w[6], pars->GC_11, pars->ZERO, pars->ZERO, w[12]); 
  FFV1_3(w[0], w[3], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  FFV1_3(w[5], w[2], pars->GC_11, pars->ZERO, pars->ZERO, w[14]); 
  FFV1_1(w[4], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[15]); 
  FFV1_2(w[1], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[16]); 
  FFV1_2(w[5], w[13], pars->GC_11, pars->ZERO, pars->ZERO, w[17]); 
...
double P0_Sigma_sm_uc_uucux_qcd_4j::matrix_uc_uucux() 
{
  int i, j; 
  // Local variables
  ...
  jamp[2] = +1./4. * (-amp[1] - std::complex<double> (0, 1) * amp[2] - amp[4] -
      ...
      amp[12]);
  jamp[3] = +1./4. * (+1./3. * amp[0] + 1./3. * amp[1] + amp[5] -
      std::complex<double> (0, 1) * amp[7] + amp[8] + amp[11] + 1./3. * amp[12]
      + 1./3. * amp[13]);
  jamp[4] = +1./4. * (+1./3. * amp[3] + 1./3. * amp[4] + 1./9. * amp[5] + 1./9.
