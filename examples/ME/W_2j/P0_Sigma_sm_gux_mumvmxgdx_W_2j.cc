...
#include "P0_Sigma_sm_gux_mumvmxgdx_W_2j.h"
...
void P0_Sigma_sm_gux_mumvmxgdx_W_2j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gux_mumvmxgdx_W_2j::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gux_wmgdx_wm_mumvmx(); 
        ...
        t[1] = matrix_gux_wmgdx_wm_mumvmx(); 
        double tsum = 0; 
...
double P0_Sigma_sm_gux_mumvmxgdx_W_2j::sigmaHat() 
{
...
void P0_Sigma_sm_gux_mumvmxgdx_W_2j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_1(w[1], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[10]); 
  FFV1_2(w[6], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[11]); 
  FFV1_2(w[6], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[12]); 
  FFV1_1(w[1], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
...
double P0_Sigma_sm_gux_mumvmxgdx_W_2j::matrix_gux_wmgdx_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 8; 
