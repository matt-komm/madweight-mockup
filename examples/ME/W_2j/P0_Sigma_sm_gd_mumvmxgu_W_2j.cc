...
#include "P0_Sigma_sm_gd_mumvmxgu_W_2j.h"
...
// Process: g d > w- g u WEIGHTED=4
// *   Decay: w- > mu- vm~ WEIGHTED=2
// Process: g s > w- g c WEIGHTED=4
// *   Decay: w- > mu- vm~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void P0_Sigma_sm_gd_mumvmxgu_W_2j::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gd_mumvmxgu_W_2j::sigmaKin() 
{
...
double P0_Sigma_sm_gd_mumvmxgu_W_2j::sigmaHat() 
{
...
void P0_Sigma_sm_gd_mumvmxgu_W_2j::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  VVV1_1(w[0], w[5], pars->GC_10, pars->ZERO, pars->ZERO, w[7]); 
  FFV2_1(w[6], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[8]); 
  FFV2_2(w[1], w[4], pars->GC_100, pars->ZERO, pars->ZERO, w[9]); 
  FFV1_1(w[6], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[10]); 
  FFV1_2(w[1], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[11]); 
  FFV1_2(w[1], w[0], pars->GC_11, pars->ZERO, pars->ZERO, w[12]); 
  FFV1_1(w[6], w[5], pars->GC_11, pars->ZERO, pars->ZERO, w[13]); 
  ...
  FFV1_0(w[9], w[10], w[5], pars->GC_11, amp[3]); 
  FFV2_0(w[12], w[13], w[4], pars->GC_100, amp[4]); 
  FFV1_0(w[12], w[8], w[5], pars->GC_11, amp[5]); 
  FFV1_0(w[9], w[13], w[0], pars->GC_11, amp[6]); 
  FFV1_0(w[11], w[8], w[0], pars->GC_11, amp[7]); 

}
double P0_Sigma_sm_gd_mumvmxgu_W_2j::matrix_gd_wmgu_wm_mumvmx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 8; 
