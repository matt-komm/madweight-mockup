...
#include "P0_Sigma_sm_gg_budxbxemvex_semiTTbar.h"
...
void P0_Sigma_sm_gg_budxbxemvex_semiTTbar::initProc(string param_card_name) 
{
...
void P0_Sigma_sm_gg_budxbxemvex_semiTTbar::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gg_ttx_t_budx_tx_bxemvex(); 
...
double P0_Sigma_sm_gg_budxbxemvex_semiTTbar::sigmaHat() 
{
...
void P0_Sigma_sm_gg_budxbxemvex_semiTTbar::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  VVV1_1(w[0], w[1], pars->GC_10, pars->ZERO, pars->ZERO, w[12]); 
  FFV1_1(w[6], w[0], pars->GC_11, pars->MT, pars->WT, w[13]); 
  FFV1_2(w[11], w[0], pars->GC_11, pars->MT, pars->WT, w[14]); 
...
double P0_Sigma_sm_gg_budxbxemvex_semiTTbar::matrix_gg_ttx_t_budx_tx_bxemvex() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 3; 
