  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ddx_zh_z_mummup_h_bbx(); 
        ...
        t[1] = matrix_ddx_zh_z_mummup_h_bbx(); 
        double tsum = 0; 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_4_3(w[3], w[2], pars->GC_50, pars->GC_59, pars->MZ, pars->WZ, w[4]); 
...
double CPPProcess::matrix_ddx_zh_z_mummup_h_bbx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 1; 
